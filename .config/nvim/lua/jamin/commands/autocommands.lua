...
vim.api.nvim_create_autocmd({ "DiagnosticChanged" }, {
  ...
  callback = function(args)
    ...
    vim.schedule(function()
      -- Replace the latest qflist if it was created by this autocmd so other qflists
      -- (e.g., vimgrep results) aren't buried due to frequently changing diagnostics.
      vim.fn.setqflist({}, qflist.title == "All Diagnostics" and "r" or " ", {
        title = "All Diagnostics",
        items = vim.diagnostic.toqflist(diagnostics),
      })

      -- Don't steal focus from other qflists. For example, when working through vimgrep results,
      -- you likely want :cnext to take you to the next match, rather than the next diagnostic.
      -- Use :cnew to switch to the diagnostic qflist when you want it.
      if qflist.id ~= 0 and qflist.title ~= "All Diagnostics" then vim.cmd.cold() end
    end)
  end,
})
